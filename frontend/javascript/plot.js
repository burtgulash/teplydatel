...
Plot.prototype.add_player(player_id) {
    this.progress[player_id] = [Tick(0, 0)];
}

Plot.prototype.update_progress(player_id, done, wpm) {
    var progress = this.progress[player_id];
    if (!progress) {
        console.log("player does not exist", {player_id: player_id});
        return
    }
